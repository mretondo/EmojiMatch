...
final class LeaderboardTableViewController: UIViewController, UITextFieldDelegate, GKGameCenterControllerDelegate {
    ...
    @IBAction private func addTheme(_ sender: UIBarButtonItem) {
        ...
        let saveAction = UIAlertAction(title: "Save", style: .default) { [weak self] _ in
            guard let self,
                  let nameTextField = self.alertController.textFields?.first,
                  let charactersTextField = self.alertController.textFields?.last else { return }
...
//            AppEnvironment.shared.themeChooserTableViewController?.insertTheme(from: themeItem)
