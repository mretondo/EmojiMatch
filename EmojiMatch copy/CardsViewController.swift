...
//    }
            ...
            for index in cardButtons.indices {
                ...
                if touchedCard != nil && (card.isTransitioningToFaceUp || card.isFaceUp) {
                    if touchedCard == index {
                        ...
                        animateFlippingCardOver(card, button)
                    } else {
    ...
    fileprivate func animateFlippingCardOver(_ card: Card, _ button: UIButton) {
        Task { @MainActor in
            let liftFinished = await liftCardUp(button)
            guard liftFinished else {
                print("Cancel Animation liftCardUp")
                return
            }
            ...
            guard flipFinished else {
                print("Cancel Animation flipCardOver")
                return
            }

            let lowerFinished = await lowerCardDown(button)
            guard lowerFinished else {
                print("Cancel Animation lowerCardDown")
                return
            }
            ...
            if card.isMatched {
                ...
                if let firstIndex = indicesOfCards.0, let secondIndex = indicesOfCards.1 {
                    ...
                    if scaleFinished {
                        ...
                        print("Cancel Animation hide cards")
                    }
                } else {
                    secondCardFlipCompleted = true
                    view.isUserInteractionEnabled = true
                ...
                if faceUpCards.count == 2 {
                    // There are two face up cards
                    #if DEBUG
                    print("There are two face up cards\n")
                    #endif
                    ...
                    // Run both cards being fliped transitions concurrently using async let
                    ...
                    if finished0 {
                        ...
                        print("Cancel Animation first card flip complete")
                    }
                    ...
                    if finished1 {
                        ...
                        print("Cancel Animation second card flip complete")
                    }
                }
            }
        }
    ...
    fileprivate func hideCard(at cardIndex: Int, _ button: UIButton) async {
        ...
        if self.gameOver.isHidden && self.game.areAllCardsMatched() {
            ...
            let gameOverFinished = await UIView.animate(
                withDuration: 2.0,
